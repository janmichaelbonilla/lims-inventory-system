...
const handler = async function(event, context) {
  console.log("⏰ Running Smart Expiry & Weekly Stock Check...");
  ...
  Object.values(inventory).forEach(item => {
    ...
    // Initialize stock tracker so it remembers "0" if all kits for this visit are used
    if (!stockCounts[study]) stockCounts[study] = {};
    if (stockCounts[study][visit] === undefined) stockCounts[study][visit] = 0;

    // Only count stock and check expiry for items currently Available
    if (item.status === 'Available') {
      stockCounts[study][visit]++;

      // Expiry Logic (Milestones + Expired)
      if (item.expiry) {
  ...
  // 4. CHECK FOR LOW STOCK (< 5) -- ONLY RUNS ON MONDAYS
  if (isMonday) {
  ...
  for (const [studyName, studyAlerts] of Object.entries(alerts)) {
    // Skip if nothing to report today
    ...
    if (recipient) {
      console.log(`📧 Sending alerts for ${studyName} to ${recipient}`);
      
      let htmlBody = `
        <div style="font-family: sans-serif; color: #333;">
          <h2 style="color: #d32f2f;">LIMS Inventory Alerts: ${studyName}</h2>
          <p>Please review the following inventory notifications.</p>
      `;

      // Inject Low Stock HTML (Mondays Only)
      if (studyAlerts.lowStock.length > 0) {
        htmlBody += `<h3 style="color: #e65100;">📉 Low Stock Warning (Weekly Report)</h3><ul>`;
      ...
      // Inject Expiring HTML
      if (studyAlerts.expiring.length > 0) {
        htmlBody += `<h3 style="color: #d32f2f;">⏳ Expiration Actions Required</h3><ul>`;
      ...
      const msg = {
        ...
        subject: `⚠️ LIMS Alert: ${studyName}`,
        html: htmlBody,
      };
